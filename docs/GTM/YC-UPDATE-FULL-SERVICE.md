...
| `docs/strategy/STRATEGY-FULL-SERVICE.md:4.3` | **Decided 2026-08-09: the ladder is the company, the $200k case is the destination.** Enterprise is unsellable until SOC 2 / MER / case studies exist and §1.2 stops being null; more importantly a big brand's creative is another *curated* set, so it reproduces the range restriction `tools/corpus/audit.py` names in the TikTok corpus, and no $200k/month advertiser will fund a 10% random arm ($20,000/month of predicted losers). SMB creative spans the full quality range and its holdout costs $60/week. **Consequences for this packet:** answers may describe the ladder ($300–$2,500/week) as the product, and must not quote §4.3's $12,000/month or its derived margins — those are still flat-fee numbers about the destination. |
| `docs/strategy/STRATEGY-FULL-SERVICE.md:4.3` (open) | The ladder tops out at $10,800/month of media, so a mid-market inbound wanting $20,000/month **cannot be quoted at all**. Either `recommend_spend()` grows a fourth tier and a custom band, or the answer is "no." Decide before the first such inbound, not after. |
