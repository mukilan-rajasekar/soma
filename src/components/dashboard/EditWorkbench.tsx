...
export default function EditWorkbench({
  ...
  sourceScore: number;
  /** Capability URL for the same batch, when the visitor wants to hand someone the share link. */
  ...
  return (
    <div className="mt-8 flex flex-col gap-10">
      {(run.shots.some((s) => s.delta !== null) ||
        run.candidates.some((c) => c.kind === "remove") ||
        run.cuts.some((c) => c.kind === "remove" || /drop shot/i.test(c.label))) && (
        <div>
          <h2 className="text-[11px] uppercase tracking-[0.12em] text-ink-3">
            Shots that drive the ranking
          </h2>
          <div className="mt-4">
            <ShotDiagnosisStrip
              shots={run.shots}
      ...
      <div className="grid gap-8 md:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <div className="min-w-0">
          ...
          {/* Before / after score strip. No second arc is inventable here: candidate cuts
              do not carry re-encoded cortical series on this host. The honest comparison
              is the measured (or estimated) number next to the original. */}
          <div className="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-line bg-line">
            <div className="bg-paper px-4 py-3">
              <div className="text-[11px] uppercase tracking-[0.1em] text-ink-3">Original</div>
              <div className="mt-1 tabular-nums text-[28px] leading-none text-ink">
                {Math.round(base)}
              </div>
            </div>
            <div className="bg-paper px-4 py-3">
              <div className="text-[11px] uppercase tracking-[0.1em] text-ink-3">
                {cut ? (cut.measured ? "Measured" : "Estimate") : "Selected"}
              </div>
              <div className="mt-1 flex items-baseline gap-2">
                <span className="tabular-nums text-[28px] leading-none text-ink">
                  {Math.round(shownScore)}
                </span>
                {cut && delta !== null ? (
                  <span className="tabular-nums text-meta text-ink-3">{fmtDelta(delta)}</span>
                ) : null}
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            {cut?.videoUrl ? (
              <a
                href={cut.videoUrl}
                download
                className="inline-block cursor-pointer rounded-xl border border-line-2 bg-paper px-4 py-[11px] text-[13px] font-medium text-ink-2 transition-colors hover:border-ink hover:text-ink"
              >
                Download this cut
              </a>
            ) : null}
